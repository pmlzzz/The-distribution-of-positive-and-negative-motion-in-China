...
function init() {
	...
	var WIDTH = window.innerWidth, HEIGHT = window.innerHeight;
		...
		document.body.appendChild(renderer.domElement);
            ...
            //console.log("testtttttttttttttttttqweqweqweqwetttt");

            $.getJSON("data/test.json", function(testdata){
                //console.log("ppp",testdata);
                $.each(testdata,function(a,b){
                    //console.log("a",a,b);
                    points[a]=b;
            ...
            //console.log("testttttttttttttttttttttt");
             ...
             $.getJSON("data/countries.json", function(corejson){
                //console.log("country",corejson.countries);
                ...
                //console.log("sphereShapeIDs",sphereShapeIDs);
            ...
            $.each(countries,function(p,v){if(i==v.id){val=v;code=p;}});
                ...
                //for(var key in val["products"]){
                    ...
                    //console.log(Math.round(productValue/dollars));
                    //productInfo=products[key];  

                    color=new THREE.Color("#7FFF00");

                    for(var s=0;s<val["particles"];s++){
                        ...
                        /*if(i!=17)
                        {
                        values_color[ v * 3 + 0 ] = 0;
                        values_color[ v * 3 + 1 ] = 0;
                        values_color[ v * 3 + 2 ] = 0;
                        }*/
                        ...
                        positions[ v * 3 + 0 ] = 5000;
                        positions[ v * 3 + 1 ] = 5000;
            ...
            console.log("asdasdasdasdasdas",countries);
...
function switcher(to,reset,incremental){
    ...
    switch(to){
                ...
                while (p < country.particles) {
                    for (var k = 0; k < IDs.length; k++) {
                        ...
                        for (var j = 0; j < countryline.geometry.vertices.length - 1; j++) {
                            ...
                            newx2=-test[0]+(2*Math.random()-1)*rand;
                            newy2=test[2]+(2*Math.random()-1)*rand;
                            newz2=test[1]+(2*Math.random()-1)*rand;
        ...
        $.each(points,function(p,o){
            point=o;

            //console.log("~~~~~~",point.id,point.LONGITUDE,point.LATITUDE,point.EMOTION,globeSize);
            theta = (point.LONGITUDE)  / 180;
            phi = (point.LATITUDE)  / 180 ;
            newx = theta * 280;//本来的-x坐标
            newz = 0;//本来的y坐标
            newy = phi * 280;//本来的z坐标
            len=Math.sqrt(newx*newx+newy*newy+newz*newz);

            destination[v * 3 + 0] =newx;
            destination[v * 3 + 1] =newy;
            destination[v * 3 + 2] =newz;
        ...
        for (var i = 0; i < 125; i++) {
            $.each(countries, function(p, o) {//console.log("iasdasdas",i);
                if (i == o.id) {
                    country = o;
                    code = p;
            ...
            IDs = country.polygons3D; //console.log("value of V：",v,country.polygons3D,country);
            var dotspacing=Math.pow(country.particles/country.area,0.5)*40;
            //console.log(country.id,"==============",IDs);
            if (IDs&&((country.id==17))){
            //if(0){
            var p = 0;
                //console.log("warningwarningwarningwarningwarning",country.id);
                while (p < country.particles) {
                    for (var k = 0; k < IDs.length; k++) {
                        ...
                        for (var j = 0; j < countryline.geometry.vertices.length - 1; j++) {
                            ...
                            newx = (vector.x+Math.random()*(vector2.x-vector.x))  ;
                            newy = -(vector.z+Math.random()*(vector2.z-vector.z) )  ;
                            newz = (vector.y+Math.random()*(vector2.y-vector.y) )  ;
                            //newx = -(Math.cos(39.9 * Math.PI/180) * Math.cos(116.3 * Math.PI/180) * 160);//本来的-x坐标
                            //newy = (Math.sin(39.9 * Math.PI/180) * 160);//本来的y坐标
                            //newz = (Math.cos(39.9 * Math.PI/180) * Math.sin(116.3 * Math.PI/180) * 160);//本来的z坐标
                            }
                            ...
                            if (p < country.particles) {
                                ...
                                if(polytest){
                                    ...
                                    //console.log("warningwarningwarningwarningwarning",Math.round(newpoint.x*dotspacing),Math.round(newpoint.x*dotspacing)/dotspacing);
                                   // destination[v * 3 + 0] =newpoint.x;
                                    //destination[v * 3 + 0] =newpoint.y;
                                    //destination[v * 3 + 0] =newpoint.z;
                                    destination[v * 3 + 0] = Math.round(newpoint.x*dotspacing)/dotspacing;
                                    ...
                                    var colorn = new THREE.Color( "#e92816" );   
                                    if(country.id==17){var colorn = new THREE.Color( "#4B0082" ); }
                                    values_color[ v * 3 + 0 ] = colorn.r*1.2;
                                    values_color[ v * 3 + 1 ] = colorn.g*1.2;
                                    values_color[ v * 3 + 2 ] = colorn.b*1.2;
                                    //var values_color = geometry.attributes.customColor.array;
                                    //console.log(v,":",destination[v * 3 + 0],destination[v * 3 + 1],destination[v * 3 + 2]);
                                }
                                v++; //console.log("value of V：",v);
                                p++;
                            } else {
                                break;
                            }
                ...
                for (var r = 0; r < country.particles; r++) {
                    destination[v * 3 + 0] = 5000;
                    destination[v * 3 + 1] = 5000;
                    destination[v * 3 + 2] = 5000;
                    geometry.attributes.customColor[v]=colorn;
                    v++;
                };
            }
        ...
        $.each(points,function(p,o){
            point=o;
           // console.log("~~~~~~",point.id,point.LONGITUDE,point.LATITUDE,point.EMOTION,globeSize);
            theta = (point.LONGITUDE-90) * Math.PI / 180;
            phi = (point.LATITUDE) * Math.PI / 180 ;
            newx = (Math.cos(phi) * Math.cos(theta) * 160);//本来的-x坐标
            newz = (Math.sin(phi) * 160);//本来的y坐标
            newy = (Math.cos(phi) * Math.sin(theta) * 160);//本来的z坐标
            len=Math.sqrt(newx*newx+newy*newy+newz*newz);

            destination[v * 3 + 0] =newx*(globeSize+5)/len;
            destination[v * 3 + 1] =newy*(globeSize+5)/len;
            destination[v * 3 + 2] =newz*(globeSize+5)/len;
            if (point.EMOTION=="b") {
                var colorn = new THREE.Color( "#e92816" );
                values_color[ v * 3 + 0 ] = colorn.r*1.2;
                values_color[ v * 3 + 1 ] = colorn.g*1.2;
                ...
                var colorn = new THREE.Color( "#7FFF00" );
                values_color[ v * 3 + 0 ] = colorn.r*1.2;
                values_color[ v * 3 + 1 ] = colorn.g*1.2;
                values_color[ v * 3 + 2 ] = colorn.b*1.2;
